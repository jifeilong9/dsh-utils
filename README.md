...
- **新建文件 / 新建文件夹 / 删除**：右键菜单新建文件 / 新建文件夹（输入行带取消按钮），可从任意文件夹右键创建；**新建文件遇重名会拒绝，绝不静默覆盖**；预览页或右键删除，删除前弹确认框；Windows 下移入**系统回收站**（Linux / macOS 为永久删除，host 端已注明）；拒绝操作根目录和 `.git` 路径
...
```sh
# 方式一：本地 tarball（构建产物）
dsh plugin --profile web add ./dsh-utils-0.6.3.tgz
