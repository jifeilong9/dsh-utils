...
- **文件名搜索**：实时搜索（精确 > 前缀 > 子串排序，自动跳过 `.git` / `node_modules`），带一键清除、Esc 退出、Enter 打开首个结果、结果计数与超量提示
...
- **新建文件 / 新建文件夹 / 删除**：右键菜单或头部"+"新建文件（输入行带取消按钮），新建文件夹可从任意文件夹右键创建；**新建文件遇重名会拒绝，绝不静默覆盖**；预览页或右键删除，删除前弹确认框；Windows 下移入**系统回收站**（Linux / macOS 为永久删除，host 端已注明）；拒绝操作根目录和 `.git` 路径
...
```sh
# 方式一：本地 tarball（构建产物）
dsh plugin --profile web add ./dsh-utils-0.6.2.tgz
