 ...
 *
 * 1. Usage badge: watches the active session's model selection; while the
 *    provider is `opencode-go`, renders a usage badge into the composer's
 *    left tool row (`conversation.input.left`) and polls the host
 *    `opencodeUsage` Remote every 10 minutes (click to refresh immediately;
 *    hovering shows monthly percent + reset countdowns).
 *
 * 2. Workspace file manager: a "文件" entry below the sidebar New Session
 ...
 *    edit-and-save with mtime conflict protection, and delete with confirm.
 *    All operations go through the workspace-gated `workspaceFiles` Remote.
 *
...
/** The opencodeUsage Remote namespace's client contribution. */
const OPENCODE_USAGE_REMOTE = {
  package: "dsh-utils",
  descriptors: [
    {
...
function UsageBadge(props) {
  ...
  const active = current !== null && current !== undefined && current.provider === "opencode-go";
  ...
  React.useEffect(() => {
    ...
    if (provider === undefined || usageCall === undefined) return undefined;
    let cancelled = false;
    let timer = null;
    const refresh = () => {
      Promise.resolve()
...
function apply(ctx) {
  ...
  // usage poll / panel open can never race the mount.
  const remotesPromise = (async () => {
    // One contribution per package: both namespaces ride a single $mount.
