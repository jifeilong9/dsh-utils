...
function FilesPanel(props) {
  ...
  const menuTerminal = () => {
    ...
    const path = menu.path;
    setMenu(null);
    Promise.resolve().then(() => api.openTerminal(root, path)).then((result) => {
  ...
  const menuVscode = () => {
    ...
    const path = menu.path;
    setMenu(null);
    Promise.resolve().then(() => api.openInVscode(root, path)).then((result) => {
      if (!result.ok) setNotice(fmt("filesError", { message: result.error.message }));
    }, (err) => {
      setNotice(fmt("filesError", { message: String(err && err.message ? err.message : err) }));
    });
  ...
  const createEntry = () => {
    ...
    const op = kind === "folder" ? api.createDir(root, path) : api.createFile(root, path);
    Promise.resolve().then(() => op).then((result) => {
      if (result.ok) {
        setCreating(null);
        setNewName("");
        loadDir(parentOf(path));
  ...
  const treeContent = searching ? hits !== null && hits.hits.length === 0 ? React.createElement("div", { className: "dsh-utils-files-hint" }, fmt("filesSearchEmpty", {})) : hits !== null && [
    ...hits.hits.map(
    ...
    hits.truncated === true && React.createElement("div", { className: "dsh-utils-files-hint", key: "trunc" }, fmt("filesSearchTruncated", {})),
  ...
  return React.createElement(
    ...
    creating !== null && React.createElement(
      "div",
      { className: "dsh-utils-files-new" },
      creating.dir !== "" && React.createElement("span", { className: "dsh-utils-files-new-path", title: creating.dir }, creating.dir + "/"),
      React.createElement("input", {
        type: "text",
        autoFocus: true,
    ...
    menu !== null && React.createElement(
      ...
      menu.path === "" && menuItemView("vscode", menuVscode, "code", false, t("filesVscode")),
      menu.isDir && menuSeparatorView("sep-open-delete"),
      menu.path !== "" && menuItemView("delete", menuDelete, "trash", false, t("filesDelete"), fmt("filesRecycleHint", {}), true)
    )
  );
}
function menuItemView(key, onClick, icon, withAdd, label, hint, danger) {
  return React.createElement(
    ...
    hint !== void 0 && hint !== "" && React.createElement("span", { className: "dsh-utils-files-menu-item-hint" }, hint)
  );
}
