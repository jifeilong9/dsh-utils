...
const WORKSPACE_FILES_SERVICE = {
  ...
  description: 'Workspace-gated file management for the web GUI: workspaces, directory listing, read/write/delete, filename search.',
  tags: [],
  members: [
...
export class WorkspaceFilesRuntime extends TypertRemoteService {
  ...
  /** Delete a path (recursive for directories); refuses the root and .git. */
  async delete(root, rel, signal) {
    ...
    try {
      ...
      throw new PanelError('write-failed', `cannot delete ${rel}`)
    }
    return { ok: true }
  }
