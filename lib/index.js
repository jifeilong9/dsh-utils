...
export const WORKSPACE_FILES_INVOCATIONS = [
  ...
  },
  {
    ...
    parameters: [jsonParam('root'), jsonParam('rel')],
    cancellation: { parameter: 'signal' },
    result: { mode: 'strict', typeSymbol: 'dsh-utils#CreateFileResult', schema: passSchema },
  },
  {
...
const WORKSPACE_FILES_SERVICE = {
  ...
  members: [
    ...
    { kind: 'method', name: 'createFile', signature: 'createFile(root: string, rel: string, signal?: AbortSignal): Promise<CreateFileResult>' },
...
export class WorkspaceFilesRuntime extends TypertRemoteService {
  ...
  /**
   * Create an empty file (right-click "new file"), refusing existing names so
   * the flow can never silently overwrite a file. Refuses the root itself and
   * .git paths.
   * @param root - workspace root.
   * @param rel - relative path of the new file (never '').
   * @returns `{ ok: true, path }` with the resulting relative path.
   */
  async createFile(root, rel, signal) {
    const canonical = await gateRoot(this.ctx, root)
    ...
    try {
      ...
      throw new PanelError('write-failed', `cannot create file ${rel}`)
    }
